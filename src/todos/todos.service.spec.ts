...
describe('TodosService', () => {
  ...
  it('findOne returns a todo by id', async () => {
    ...
    expect(result).toEqual(mockTodo);
    expect(mockPrismaService.todo.findUnique).toHaveBeenCalledWith({ where: { id: 'uuid-1' } });
  });
  ...
  it('update throws NotFoundException when not found', async () => {
    mockPrismaService.todo.findUnique.mockResolvedValue(null);
    await expect(service.update('missing', { completed: true })).rejects.toThrow(NotFoundException);
  });
  ...
  it('remove calls delete and returns the deleted todo', async () => {
    ...
    expect(result).toEqual(mockTodo);
    expect(mockPrismaService.todo.delete).toHaveBeenCalledWith({ where: { id: 'uuid-1' } });
  });
